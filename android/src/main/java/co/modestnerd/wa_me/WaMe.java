...
public class WaMe implements FlutterPlugin, MethodCallHandler {
    ...
    private void share(MethodCall call, Result result) {
        ...
        Log.i("WaMe", "title: " + title);
        Log.i("WaMe", "text: " + text);
        Log.i("WaMe", "linkUrl: " + linkUrl);
        Log.i("WaMe", "phone: " + phone);
        Log.i("WaMe", "packageName: " + packageName);

        if (TextUtils.isEmpty(title)) {
            result.error("Title cannot be null or empty", null, null);
            return;
        }

        if (TextUtils.isEmpty(phone)) {
            result.error("Phone cannot be null or empty", null, null);
            return;
        }

        if (TextUtils.isEmpty(packageName)) {
            result.error("Package name cannot be null or empty", null, null);
            return;
        }

        if (TextUtils.isEmpty(text) && TextUtils.isEmpty(linkUrl)) {
            result.error("Text and linkUrl cannot be null or empty", null, null);
            return;
        }
        ...
        try {
            ...
            // 🔥 detect Samsung
            boolean isSamsung =
                    (Build.MANUFACTURER != null && Build.MANUFACTURER.toLowerCase().contains("samsung")) ||
                    (Build.BRAND != null && Build.BRAND.toLowerCase().contains("samsung"));

            Log.i("WaMe", "isSamsung: " + isSamsung);

            if (isSamsung) {
                // 🔥 aman → chooser (hindari silent fail clone)
                Intent chooser = Intent.createChooser(
                        intent,
                        TextUtils.isEmpty(chooserTitle) ? "Pilih WhatsApp" : chooserTitle
                );
                chooser.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                context.startActivity(chooser);
            } else {
                // 🔥 direct (device non-samsung)
                intent.setPackage(packageName);
                context.startActivity(intent);
            }
            ...
            Log.e("WaMe", "No app available", ex);
            result.error("No app available to handle intent", null, null);
        } catch (Exception ex) {
            Log.e("WaMe", "Error sharing message", ex);
            result.error("Error sharing message: " + ex.getMessage(), null, null);
        }
    }
    ...
    private void shareFile(MethodCall call, Result result) {
        ...
        String filePath = call.argument("filePath");
        String phone = call.argument("phone");
        String targetPackage = call.argument("package"); // 🔥 tambahan

        Log.i("WaMe", "text: " + text);
        Log.i("WaMe", "phone: " + phone);
        Log.i("WaMe", "filePath: " + filePath);
        Log.i("WaMe", "targetPackage: " + targetPackage);

        if (TextUtils.isEmpty(filePath)) {
            result.error("FilePath cannot be null or empty", null, null);
            return;
        }
        ...
        try {
            ...
            if (isSamsung) {
                // 🔥 aman → chooser (hindari silent fail clone)
                Intent chooser = Intent.createChooser(intent, "Pilih WhatsApp");
                chooser.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                context.startActivity(chooser);
            } else {
                // 🔥 direct (device non-samsung)
            ...
            Log.e("WaMe", "Error sharing file", ex);
            result.error("Error sharing file: " + ex.getMessage(), null, null);
        }
    }
}
